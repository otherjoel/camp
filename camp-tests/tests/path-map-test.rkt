...
(check-false (output-path-pattern? "foo/bar"))        ; must contain at least one *
(check-false (output-path-pattern? "foo"))            ; must contain at least one *

;; Invalid CLDR patterns in brackets should be rejected
(check-false (output-path-pattern? "blog/[invalid]/*/"))
(check-false (output-path-pattern? "blog/[zzz]/*/"))
(check-false (output-path-pattern? "blog/[hello-world]/*/"))
...
;; Basic slug substitution (no dates)
(check-equal? (format-output-path "*" "my-post" #f)
              (string->path "my-post.html"))
(check-equal? (format-output-path "posts/*" "my-post" #f)
...
(check-equal? (format-output-path "[yyyy]/[MM]/[dd]/*" "my-post" test-date)
              (string->path "2025/01/15/my-post.html"))

;; Single-digit date formats
(check-equal? (format-output-path "blog/[y]/[M]/[d]/*" "my-post" (date 2025 3 5))
              (string->path "blog/2025/3/5/my-post.html"))

;; Abbreviated month name
(check-equal? (format-output-path "archive/[MMM]/*" "my-post" test-date)
              (string->path "archive/Jan/my-post.html"))

;; Complex pattern with trailing slash
(check-equal? (format-output-path "archive/[yyyy]-[MM]/*/" "my-post" test-date)
              (string->path "archive/2025-01/my-post/index.html"))

;; Error when date patterns present but no date provided
(check-exn exn:fail?
           (lambda () (format-output-path "blog/[yyyy]/*" "my-post" #f))
           "Should error when date codes present but no date given")

(check-exn exn:fail?
           (lambda () (format-output-path "[MM]/[dd]/*/" "my-post" #f))
